...
namespace MidnightRacer.GameObjects
{
    ...
    {
        protected override VectorGroup Bounds { get; set; } = VectorGroup.FromRect(new Size(40, 40));
        ...
        public override void Render()
        {
            ...
            pts[3] = new Vector(pts[3].X, pts[3].Y - 8);
            var canPts = new VectorGroup(pts[0], pts[1], pts[2], pts[3], pts[1], pts[2], pts[0]);

            View.FillPolygon(new VectorGroup(pts).Move(Position), color, false);
            View.DrawPolygon(canPts.Move(Position), Color.Black, false);
        }

        public PetrolCan()
        {
            var test = new Random();
            color = Color.FromArgb(test.Next(0, 150), test.Next(0, 150), test.Next(0, 150));
        }
